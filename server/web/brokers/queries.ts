...
export const getTrustedPlatforms = async (limit: number = 7) => {
  const category = await getCategoryWithBrokers("trusted-trading-platforms", limit)
  const brokers = await Promise.all(
    (category?.brokers || []).map(async (broker) => ({
      ...
      logo: await getBrokerLogo(broker),
...
export const getAllPartners = async (limit: number = 40) => {
  const brokers = await db.brokers.findMany({
    ...
    orderBy: [{ isSponsor: "desc" }, { year_established: "desc" }, { broker_name: "asc" }],
    take: limit,
  })
