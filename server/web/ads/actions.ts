...
import {
  createAdDetailsSchema,
  createPreCheckoutAdSchema,
...
export const createAdFromCheckout = actionClient
  ...
  .action(
    async ({
      parsedInput: { sessionId, ...adDetails },
      ctx: { db, revalidate },
    }) => {
      ...
      if (adDetails.subcategoryId) {
        const sub = await db.subcategory.findUnique({
          where: { id: adDetails.subcategoryId },
        });
        if (!sub) {
          // If the provided subcategory doesn't exist, ignore it to avoid a FK violation
          console.warn(
            `Invalid subcategoryId provided in ad details: ${adDetails.subcategoryId} — ignoring`,
          );
    ...
    async ({
      ...
      const cregisResponse = await cregis.createCheckout({
        ...
        cancel_url: cancelUrl
          ? `${siteConfig.url}${cancelUrl}?cancelled=true`
          : undefined,
      });
      console.log("🚀 ~ cregisResponse:", cregisResponse);

      if (
        cregisResponse.code !== "00000" ||
        !cregisResponse.data.checkout_url
      ) {
      ...
      const resolvedFaviconUrl =
        faviconUrl ||
        (await fetchAndUploadMedia(websiteUrl, faviconPath, "favicon"));
      ...
      if (subcategoryId) {
        ...
        if (!sub) {
          console.warn(
            `Invalid subcategoryId provided when creating draft ad: ${subcategoryId} — ignoring`,
          );
          resolvedSubcategoryId = undefined;
        } else {
      ...
      if (parsedAds.length > 0) {
        try {
          await db.$transaction(
            parsedAds.map((ad) =>
              db.ad.create({ data: { ...ad, ...adDetails } }),
            ),
          );
        } catch (err) {
  ...
  .action(async ({ parsedInput: { adId }, ctx: { db } }) => {
    try {
      ...
      if (ad.status !== "Scheduled") {
        console.warn(
          `Attempted to track click for non-active ad: ${adId}, status: ${ad.status}`,
        );
...
export const trackAdImpression = actionClient
  ...
  .action(async ({ parsedInput: { adId }, ctx: { db } }) => {
    try {
      ...
      if (ad.status !== "Scheduled") {
        console.warn(
          `Attempted to track impression for non-active ad: ${adId}, status: ${ad.status}`,
        );
        return { success: false, error: "Ad is not active" };
      }
