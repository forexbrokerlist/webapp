...
const TableRow = ({ className, ...props }: ComponentProps<"tr">) => {
  return (
    <tr
      className={cx(
        "relative grid grid-cols-subgrid col-span-full items-center min-h-9 py-1 overflow-clip [tbody>&:not([aria-disabled])]:hover:bg-accent data-[state=selected]:bg-accent",
        className,
      )}
      {...props}
    />
