...
export default function NewsCardList({ activeSource }: NewsCardListProps) {
    ...
    const SkeletonCard = () => (
        <div className='border border-[rgba(26,26,26,0.14)] bg-[rgba(255,255,255,0.50)] p-3 rounded-xl animate-pulse'>
            <div className='h-[250px] w-full rounded-xl bg-gray-200' />
            ...
            <div className="max-w-[1640px] px-5 mx-auto max-laptop:px-16 max-tab:px-5 max-mobile:px-4">
                <div className='grid grid-cols-3 pb-14 gap-5'>
                    {loading ? (
                        ...
                        news?.length > 0 ? news.map((item: any, i: number) => {
                            return (
                                <div key={i} onClick={() => fetchDetails(item.id)} className='border border-[rgba(26,26,26,0.14)] bg-[rgba(255,255,255,0.50)] p-3 rounded-xl cursor-pointer'>
                                    ...
                                    <div className='relative h-[250px] w-full rounded-xl overflow-hidden'>
                                        ...
                                        <div className='absolute top-4 left-4 bg-[rgba(26,26,26,0.8)] backdrop-blur-sm text-white text-sm font-medium py-1.5 px-4 rounded-full z-20'>
                                    ...
                                    <div className='pt-5'>
                                        ...
                                        <div className='grid grid-cols-2 gap-3 pt-2'>
                                            ...
                                            <Button
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    if (!item.analyzed) analyzeNews(item.id);
                                                }}
                                                className={`px-5 gap-2.5 group rounded-full relative z-[9] border transition-all
                                                    ${item.analyzed
                                                        ? 'opacity-50 cursor-not-allowed bg-[#F0F1EC] border-[rgba(26,26,26,0.10)] text-black100'
                                                        : 'hover:bg-white text-black100 border-[rgba(26,26,26,0.10)] bg-[#F0F1EC]'
                                                    }`}
                                            >
                ...
                <div className="flex items-center justify-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-5 cursor-pointer py-2.5 bg-white border border-solid border-border-light300 rounded-full text-black700 font-medium hover:bg-primary hover:border-primary transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    <div className="flex items-center gap-2">
                        {(() => {
                            ...
                            return pages.map((p, i) =>
                                p === '...'
                                    ? <span key={`ellipsis-${i}`} className="w-12 h-12 flex items-center justify-center text-black700 font-medium">...</span>
                                    : <button
                                        key={p}
                                        onClick={() => setCurrentPage(p as number)}
                                        className={`w-12 h-12 flex items-center justify-center rounded-full font-medium transition-all
                                            ${currentPage === p
                                                ? 'bg-primary text-black100 font-bold text-lg shadow-sm'
