...
import { findBrokerBySlug, findRandomCrmProviders, findCrmProvidersForComparison } from "~/server/web/tools/queries"
import { getPresignedUrlFromFull, getScreenshotFetchUrl } from "~/lib/media"
import ForexCrmDetails from "~/components/web/forex-crm-details"
...
const getData = cache(async ({ params }: Props) => {
  ...
  const url = `/forex-crm-providers/${broker.slug}`
  ...
  const data = getPageData(url, title, description, {
    ...
    structuredData: [
      ...
      generateBrokerFAQ(broker, "Forex CRM Provider"),
  ...
  const randomBrokersRaw = await findRandomCrmProviders(3, slug)
  ...
  const trustedBrokersRaw = await findCrmProvidersForComparison(12)
...
export default async function (props: Props) {
  ...
  return (
    <>
      <ForexCrmDetails broker={broker} randomBrokers={randomBrokers} trustedBrokers={trustedBrokers} />
