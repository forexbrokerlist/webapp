...
import { findBrokerById } from "~/server/web/tools/queries"

type Props = PageProps<"/brokers/[id]">
...
const getData = cache(async ({ params }: Props) => {
  ...
  const broker = await findBrokerById(brokerId)
        ...
        <Section.Content className="max-md:contents">
          <Sticky isOverlay>
            <Stack className="@container self-stretch justify-between items-center">
              ...
              {broker.url && (
                <a href={broker.url} target="_blank" rel="noopener noreferrer nofollow" className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                  Visit Broker
                </a>
              )}
              <Backdrop />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            ...
            {/* Trading Conditions */}
            {(broker.daily_loss_limit || broker.retail_loss_rate || broker.maximum_trailing_drawdown || broker.trading_hours || broker.minimum_raw_spreads || broker.minimum_standard_spreads || broker.minimum_commission_for_forex || broker.profit_share) && (
              <Card className="p-0 overflow-hidden shadow-sm h-full">
                <CardHeader className="bg-muted/30 py-4 px-6 border-b">
            ...
            {/* Accounts & Funding */}
            {(broker.deposit_options || broker.withdrawal_options || broker.funding_methods || broker.funded_account_options || broker.minimum_evaluation_fee || broker.minimum_funded_account || broker.maximum_funded_account) && (
              <Card className="p-0 overflow-hidden shadow-sm h-full">
                <CardHeader className="bg-muted/30 py-4 px-6 border-b">
            ...
            {/* Trading Costs */}
            {(broker.average_trading_cost_eur_usd || broker.average_trading_cost_gbp_usd || broker.average_trading_cost_gold || broker.average_trading_cost_bitcoin || broker.average_trading_cost_wti_crude_oil) && (
              <Card className="p-0 overflow-hidden shadow-sm h-full">
                <CardHeader className="bg-muted/30 py-4 px-6 border-b">
            ...
            {/* Platform & Features */}
            {(broker.trading_platforms || broker.mobile_app_comparison || broker.other_fees || broker.trader_table || broker.regulators_table) && (
