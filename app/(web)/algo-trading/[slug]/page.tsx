...
import { findBrokerBySlug, findRandomAlgoProviders,findAlgoProvidersForComparison } from "~/server/web/tools/queries"
import { getPresignedUrlFromFull, getScreenshotFetchUrl } from "~/lib/media"
import ForexAlgoProviderDetails from "~/components/web/forex-algo-provider-details"
...
const getData = cache(async ({ params }: Props) => {
  ...
  const data = getPageData(url, title, description, {
    ...
    structuredData: [
      ...
      generateBrokerFAQ(broker, "Algo Trading Provider"),
 ...
 const randomBrokersRaw = await findRandomAlgoProviders(3, slug)
    const randomBrokers = await Promise.all(
      ...
      const trustedBrokersRaw = await findAlgoProvidersForComparison(12)
...
export const generateMetadata = async (props: Props): Promise<Metadata> => {
  const { broker, url, metadata,randomBrokers } = await getData(props)
...
export default async function (props: Props) {
  ...
  return (
    <>
      ...
      <ForexAlgoProviderDetails broker={broker} randomBrokers={randomBrokers} trustedBrokers={trustedBrokers} />
