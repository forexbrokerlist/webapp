...
async function main() {
  ...
  for (const record of records) {
    ...
    const mappedData: any = {
      ...
      broker_name: record.company_name || null,
   typeId: "cmo106vqf00072ci1opc5w3fd", 
  categories: {
    connect: [
      { id: "cmmg85ful0001tml95ubdraqc" }
    ]
  },
      ...
      headquarters: record.country || null,
      ...
      url: record.company_website || null,
      withdrawal_options: record.withdrawal_options || null,
      year_established: record.year_founded ? parseFloat(record.year_founded) : null,
      ...
      broker_website: record.company_website || null,
      slug: record.company_name ? record.company_name.toString().toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') : null,
    };
